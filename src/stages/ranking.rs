...
fn merge_rankings(rankings: &[Vec<String>]) -> Vec<String> {
    ...
    let all_files: HashSet<String> = rankings
        .iter()
    ...
    for ranking in rankings {
        ...
        for file in all_files.iter() {
            if !ranking.contains(file) {
                file_to_relative_ranks.get_mut(file).unwrap().push(1.0);
            }
        }
    }

    // Calculate average relative rank for each file
    let mut files_with_rank: Vec<(String, f64)> = file_to_relative_ranks
        .into_iter()
        .map(|(file, ranks)| {
    ...
    files_with_rank.into_iter().map(|(file, _)| file).collect()
}
...
async fn rank_problem_files(
    ...
    // Merge the rankings
    let final_ranking = merge_rankings(
        &rankings
            .iter()
            .map(|r| r.ranking.clone())
            .collect::<Vec<_>>(),
    );
