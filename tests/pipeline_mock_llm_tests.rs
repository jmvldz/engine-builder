...
impl LLMClient for MockLLMClient {
    async fn completion(
        ...
        let content = self
            ...
            .get(&key)
            .cloned()
            .unwrap_or_else(|| format!("Mock response not found for key: {}", key));
...
async fn test_mock_pipeline_flow() -> Result<()> {
    ...
    // Create test configs
    let (global_config, _relevance_config, codebase_config, _ranking_config) =
        create_test_configs();
    ...
    assert!(file_patterns
        .patterns
        .contains(&"src/models/file.rs".to_string()));
    ...
    let response_path = trajectory_store
        .problem_dir()
...
These files are most likely to be relevant to the issue described."#,
    ...
    for path in ["src/main.rs", "src/lib.rs", "src/models/file.rs"].iter() {
        ...
        assert_eq!(
            ...
            engine_builder::models::relevance::RelevanceStatus::Relevant
        );
    ...
    let ranking = engine_builder::models::ranking::ProblemContext {
        model_rankings: vec![engine_builder::models::ranking::FileRanking {
