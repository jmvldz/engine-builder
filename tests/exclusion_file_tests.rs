...
fn test_loading_exclusion_file() {
    ...
    file.write_all(exclusion_content.as_bytes()).expect("Failed to write to exclusion file");
    ...
    assert_eq!(exclusion_config.extensions_to_skip.len(), 3, "Should have 3 extensions to skip");
    assert!(exclusion_config.extensions_to_skip.contains(&".png".to_string()), "Should contain .png");
    assert!(exclusion_config.extensions_to_skip.contains(&".jpg".to_string()), "Should contain .jpg");
    assert!(exclusion_config.extensions_to_skip.contains(&".pdf".to_string()), "Should contain .pdf");
    
    assert_eq!(exclusion_config.files_to_skip.len(), 2, "Should have 2 files to skip");
    assert!(exclusion_config.files_to_skip.contains(&"package-lock.json".to_string()), "Should contain package-lock.json");
    assert!(exclusion_config.files_to_skip.contains(&".DS_Store".to_string()), "Should contain .DS_Store");
    
    assert_eq!(exclusion_config.directories_to_skip.len(), 3, "Should have 3 directories to skip");
    assert!(exclusion_config.directories_to_skip.contains(&".git".to_string()), "Should contain .git");
    assert!(exclusion_config.directories_to_skip.contains(&"node_modules".to_string()), "Should contain node_modules");
    assert!(exclusion_config.directories_to_skip.contains(&"dist".to_string()), "Should contain dist");
