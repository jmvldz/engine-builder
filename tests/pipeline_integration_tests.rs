...
use engine_builder::models::ranking::{ProblemContext, RankedCodebaseFile, RelevantFileDataForPrompt};
...
fn test_full_pipeline_format_compatibility() -> Result<()> {
    ...
    let file_patterns_json = std::fs::read_to_string(store.problem_dir().join("file_patterns.json"))?;
