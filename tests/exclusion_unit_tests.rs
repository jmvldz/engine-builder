...
fn test_should_exclude_git_directory() {
    ...
    let mut git_subfile =
        File::create(&git_subfile_path).expect("Failed to create .git/objects/somehash file");
    git_subfile
        .write_all(b"object data")
        .expect("Failed to write to .git/objects/somehash file");
    ...
    assert!(
        problem.should_exclude(&git_dir_entry),
    ...
    assert!(
        problem.should_exclude(&git_file_entry),
        "File inside .git directory should be excluded"
    );

    // Test subdirectory inside .git
    let git_subdir_entry = create_dir_entry(&git_subdir_path);
    assert!(
        problem.should_exclude(&git_subdir_entry),
        "Subdirectory inside .git should be excluded"
    );

    // Test file inside subdirectory inside .git
    let git_subfile_entry = create_dir_entry(&git_subfile_path);
    assert!(
        problem.should_exclude(&git_subfile_entry),
        "File inside .git subdirectory should be excluded"
    );

    // Test normal directory and file (should not be excluded)
    let src_dir_entry = create_dir_entry(&temp_path.join("src"));
    assert!(
        !problem.should_exclude(&src_dir_entry),
        "Normal directory should not be excluded"
    );

    let src_file_entry = create_dir_entry(&src_file_path);
    assert!(
        !problem.should_exclude(&src_file_entry),
...
fn test_should_exclude_gitignore_patterns() {
    ...
    let mut node_modules_file =
        File::create(&node_modules_file_path).expect("Failed to create node_modules file");
    node_modules_file
        .write_all(b"{}")
        .expect("Failed to write to node_modules file");
    ...
    let mut problem =
        SWEBenchProblem::new("test-gitignore".to_string(), "Test gitignore".to_string())
            .with_codebase_path(temp_path)
            .with_exclusion_config(exclusion_config);

    println!(
        "Before initialization, checking if .gitignore exists: {}",
        temp_path.join(".gitignore").exists()
    );
    ...
    let node_modules_files: Vec<&String> = all_files
        .iter()
        .filter(|path| path.contains("node_modules"))
        .collect();
    println!(
        "Files with 'node_modules' in path: {}",
    ...
    assert_eq!(
        ...
        "Files in target/ should not be in all_file_paths()"
    );
    assert_eq!(
        node_modules_files_count, 0,
        "Files in node_modules/ should not be in all_file_paths()"
    );
    assert_eq!(
        log_files_count, 0,
        "Log files should not be in all_file_paths()"
    );
    ...
    println!(
        "node_modules dir excluded: {}",
        problem.should_exclude(&create_dir_entry(&temp_path.join("node_modules")))
    );
    println!(
        "log file excluded: {}",
        problem.should_exclude(&create_dir_entry(&log_file_path))
    );
    ...
    assert!(
        !problem.should_exclude(&readme_file_entry),
