...
fn test_save_and_load_relevance_decision() {
    ...
    // Save the decision
    store.save_per_file_relevance_decision(file_path, decision.clone()).unwrap();
...
fn test_load_all_relevance_decisions() {
    ...
    test_decisions.insert("file1.rs".to_string(), RelevanceDecision::relevant(
        "Message 1".to_string(),
        "Explanation 1".to_string()
    ));
    test_decisions.insert("file2.rs".to_string(), RelevanceDecision::relevant(
        "Message 2".to_string(),
