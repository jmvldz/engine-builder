...
async fn test_exclusion_patterns() {
    ...
    exclusion_config
        .directories_to_skip
        .retain(|dir| dir != "tests");

    // Apply the exclusion filter to the mock files
    let filtered_files: Vec<String> = mock_entries
        .iter()
        ...
        .filter_map(|entry| {
            entry
                ...
                .ok() // Remove the root path prefix
    ...
    assert!(
        ...
        "Should contain src/main.rs. Found paths: {:?}",
        file_paths_set
    );
    assert!(
        file_paths_set.contains("docs/readme.md"),
        "Should contain docs/readme.md. Found paths: {:?}",
        file_paths_set
    );
    assert!(
        file_paths_set.contains("README.md"),
        "Should contain README.md. Found paths: {:?}",
        file_paths_set
    );

    // Verify files with excluded extensions are not included
    assert!(
        !file_paths_set.contains("assets/logo.png"),
        "Should not contain assets/logo.png"
    );
    assert!(
        !file_paths_set.contains("assets/sound.mp3"),
        "Should not contain assets/sound.mp3"
    );
    assert!(
        !file_paths_set.contains("docs/manual.pdf"),
        "Should not contain docs/manual.pdf"
    );
    assert!(
        !file_paths_set.contains("src/script.min.js"),
        "Should not contain src/script.min.js"
    );

    // Verify excluded files by name are not included
    assert!(
        !file_paths_set.contains("package-lock.json"),
        "Should not contain package-lock.json"
    );

    // Verify files in excluded directories are not included
    assert!(
        !file_paths_set.contains(".git/config"),
        "Should not contain .git/config"
    );
    assert!(
        !file_paths_set.contains(".vscode/settings.json"),
        "Should not contain .vscode/settings.json"
    );
    assert!(
        !file_paths_set.contains("node_modules/package.json"),
        "Should not contain node_modules/package.json"
    );

    // Verify no paths start with the excluded directory prefixes
    assert!(
        file_paths_set.iter().all(|path| !path.starts_with(".git/")),
        "No paths should start with .git/"
    );
    assert!(
        file_paths_set
            .iter()
            .all(|path| !path.starts_with(".vscode/")),
        "No paths should start with .vscode/"
    );
    assert!(
        file_paths_set
            .iter()
    ...
    assert!(
        file_paths_set.iter().all(|path| !path.ends_with(".png")),
        "No paths should end with .png"
    );
    assert!(
        file_paths_set.iter().all(|path| !path.ends_with(".mp3")),
        "No paths should end with .mp3"
    );
    assert!(
        file_paths_set.iter().all(|path| !path.ends_with(".pdf")),
        "No paths should end with .pdf"
    );
    assert!(
        file_paths_set.iter().all(|path| !path.contains(".min.js")),
        "No paths should contain .min.js"
    );
}
