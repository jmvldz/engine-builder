use engine_builder::config::{Config, ContainerConfig, LLMConfig};
use std::fs::File;
use std::io::Write;
...
fn test_config_default() {
    ...
    assert_eq!(
        default_config.get_model_for_stage(&default_config.relevance.model),
        "claude-3-7-sonnet-20250219"
    );
    assert_eq!(
        default_config
            .to_llm_config(&default_config.relevance.model)
            .model_type,
        "anthropic"
    );
...
fn test_config_from_file() {
    ...
    assert_eq!(
        config.get_model_for_stage(&config.relevance.model),
        "test_model"
    );
